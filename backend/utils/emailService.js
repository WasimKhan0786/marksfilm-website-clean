// Email service for Marks Film
// Send notifications and confirmations via Gmail SMTP
...
const sendContactNotification = async (contactData) => {
    try {
        ...
        const mailOptions = {
            from: `"Marks Film Website" <${process.env.EMAIL_USER}>`,
...
const sendBookingConfirmation = async (bookingData) => {
    try {
        ...
        const customerMailOptions = {
            from: `"Marks Film" <${process.env.EMAIL_USER}>`,
            to: customer_email,
        ...
        console.log('✅ Booking confirmation sent to customer:', customerInfo.messageId);
        console.log('✅ Booking notification sent to admin:', adminInfo.messageId);
...
const sendPaymentNotification = async (paymentData) => {
    try {
        ...
        // Instant notification to admin (aapko)
        const adminMailOptions = {
            from: `"Marks Film Payment Alert" <${process.env.EMAIL_USER}>`,
            to: process.env.EMAIL_USER,
        ...
        // Send both emails simultaneously for real-time notification
        const [adminResult, customerResult] = await Promise.all([
            transporter.sendMail(adminMailOptions),
            transporter.sendMail(customerMailOptions)
        ]);

        console.log('🚀 REAL-TIME PAYMENT NOTIFICATION SENT!');
